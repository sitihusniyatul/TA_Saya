...
function [uo,dps] = of_he(x)
    ...
    kinviss = viss/dens; %kinematic viscosity
    cps = 6.842; %specific heat
    ks = 0.019; %thermal conductivity
    prs = viss*cps/ks; %prandtl number
    rfs = 0.000035; %shell fouling factor
    ...
    kinvist = vist/dent; %kinematic viscosity
    cpt = 15.875; %specific heat
    kt = 0.654; %thermal conductivity
    prt = vist*cpt/kt; %prandtl number
    rft = 0.000018; %tube fouling factor
    
    %% tube mechanical data
    np = 2; %number of pass
    nt = 6; %number of tubes
    tle = 762; %tube effective length
